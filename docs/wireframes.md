...
```mermaid
graph TD
    A["랜딩 페이지 접속<br>(mylink.com/닉네임)"] --> B{"현재 접속자 상태(Auth)"}
    
    %% 분기 1: 단순 방문자
    B -- "비로그인 (또는 타인 접속)" --> C["방문자 뷰<br>(Visitor View)"]
    C --> C1["프로필 열람<br>(photoUrl, Bio)"]
    C --> C2["링크 버튼 클릭"]
    C2 --> C3["외부 목적지 사이트 랜딩<br>(새 창)"]
    
    %% 분기 2: 소유자 대시보드
    B -- "소셜 로그인 성공" --> O{"신규 유저<br>(DB 없음)"}
    O -- "Yes (최초 로그인)" --> P["온보딩 뷰<br>(닉네임 설정)"]
    P -- "닉네임 저장 및 DB 등록" --> D["대시보드 뷰<br>(Owner View)"]
    O -- "No (기존 유저)" --> D
    
    D --> E["프로필 영역 인라인 편집"]
    E --> F["DB 반영<br>(displayName, bio)"]
    
    D --> G["+ 링크 추가 버튼 클릭"]
    G --> H["새로운 빈 링크 카드 생성"]
    H --> I["인라인 제목/URL 입력<br>(저장 시 DB 반영)"]
